...
public class ObstacleCourse {
	...
	private boolean findExit(int row, int col) {
		...
		boolean onBorder = row == course.length - 1 || row == 0 || col == course[0].length - 1 || col == 0;
		if (onBorder) {
			foundRow = row;
			foundCol = col;
		...
		// vertical
		if (course[row + 1][col] != TRIED && course[row + 1][col] != '+') {
			boolean found = findExit(row + 1, col);
			if (found) {
				course[row][col] = PART_OF_PATH;
				return true;
			}
		...
		// horizontal
		if (course[row][col + 1] != TRIED && course[row][col + 1] != '+') {
			boolean found = findExit(row, col + 1);
			if (found) {
				course[row][col] = PART_OF_PATH;
				return true;
			}
